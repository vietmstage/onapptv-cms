...
export default class SeriesList extends Component {
  ...
  _handleArchive = () => {
    ...
    updateSeriesMany(
      ...
      if (result && !result.errors) {
        toast.success(`Series [${archivedItem.title}] archived successfully.`)
        this.setState({selected: [], isArchivingIds: [], archivedItem: {}})
        this._handleGetSeries()
      } else {
        toast.error(`Series [${archivedItem.title}] archived failed.`)
      }
    })
  }
  ...
  _handleBulkArchive = () => {
    ...
    updateSeriesMany(
      ...
      if (result && !result.errors) {
        toast.success(`[${selected.length}] selected series archived successfully.`)
        this.setState({selected: [], isArchivingIds: [], archivedItem: {}})
        this._handleGetSeries()
      } else {
        toast.error(`Archived series failed.`)
      }
    })
  }
    ...
    return (
      <div>
        ...
        {!!items.length && <div style={{position: 'relative'}}>
          ...
          <Confirm
            open={showConfirm}
            content={`Are you sure to archive series [${this.state.archivedItem.title || ''}] ?`}
            cancelButton='No'
            confirmButton='Yes'
          ...
          <Confirm
            ...
            content={`Are you sure to archive all these ${selected.length} selected series?`}
            cancelButton='No'
            confirmButton='Yes'
